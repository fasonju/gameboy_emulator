...
impl Cpu {
    ...
    fn fetch_instruction(&mut self, memory: &Memory) -> Instruction {
        ...
        match ((xx, yy, zzzz), (xx, aaa, bbb), (iii, jj, bbb)) {
            ...
            ((0x0, _, 0x1), _, _) => Instruction::LdR16Imm16(R16::from(yy), self.fetch_word(memory)), // LD R16, imm16
            ...
            (_, _, (0x1, _, 0x0)) => Instruction::JrCondImm8(COND::from(jj), self.fetch_byte(memory)), // JR cond, imm8
            ...
            (_, _, (0x6, _, 0x2)) => Instruction::JpCondImm16(COND::from(jj), self.fetch_word(memory)), // JP cond, imm16
            (_, _, (0x6, 0x0, 0x3)) => Instruction::JpImm16(self.fetch_word(memory)), // JP imm16
            (_, _, (0x7, 0x1, 0x1)) => Instruction::JpHl, // JP HL
            (_, _, (0x6, _, 0x4)) => Instruction::CallCondImm16(COND::from(jj), self.fetch_word(memory)), // CALL cond, imm16
...
mod tests {
    ...
    fn test_fetch_instruction() {
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdR16Imm16(R16::BC, 0x12));

        memory.write_byte(4, 0x02);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdR16MemA(R16MEM::BC));

        memory.write_byte(5, 0x0A);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdAR16Mem(R16MEM::BC));

        memory.write_byte(6, 0x08);
        memory.write_byte(7, 0x34);
        memory.write_byte(8, 0x12);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdMemImm16SP(0x1234));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::AddHlR16(R16::BC));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdR8Imm8(R8::B, 0x12));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::JrCondImm8(COND::NotZero, 0x10));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::LdR8R8(R8::B, R8::B));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::RetCond(COND::NotZero));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::JpCondImm16(COND::NotZero, 0x3412));
        ...
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::CallCondImm16(COND::NotZero, 0x3412));

        memory.write_byte(68, 0xCD);
        memory.write_byte(69, 0x12);
        memory.write_byte(70, 0x34);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::CallImm16(0x3412));

        memory.write_byte(71, 0xC7);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::RstTgt3(TGT3::ZERO));

        memory.write_byte(72, 0xC1);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::PopR16Stk(R16STK::BC));

        memory.write_byte(73, 0xC5);
        assert_eq!(cpu.fetch_instruction(&memory), Instruction::PushR16Stk(R16STK::BC));
